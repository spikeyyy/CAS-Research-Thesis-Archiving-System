...
<nav class="bg-navy w-100 px-2 py-1 position-fixed top-0" id="login-nav">
  <div class="d-flex justify-content-between w-100">
    <div>
      <span class="mr-2  text-white"><i class="fa fa-phone mr-1"></i>
    ...
    <div>
      <?php if ($_settings->userdata('id') > 0): ?>
        ...
        <span class="mx-2">Howdy,
          <?= !empty($_settings->userdata('email')) ? $_settings->userdata('email') : $_settings->userdata('username') ?>
        </span>
...
<nav class="main-header navbar navbar-expand navbar-light border-0 navbar-light text-sm" id='top-Nav'>
  ...
  <div class="container">
    ...
    <div class="collapse navbar-collapse order-3" id="navbarCollapse">
      ...
      <li class="nav-item dropdown">
        <a id="dropdownSubMenu1" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
          class="nav-link dropdown-toggle <?= isset($page) && $page == 'projects_per_year' ? "active" : "" ?>">Year</a>
        <ul aria-labelledby="dropdownSubMenu1" class="dropdown-menu border-0 shadow" style="left: 0px; right: inherit;">
          <?php
          $currentYear = date('Y');
          ...
          for ($year = $currentYear; $year >= $startYear; $year--) {
            ?>
            <li>
              <a href="./?page=projects_per_year&year=<?= $year ?>" class="dropdown-item"><?= $year ?></a>
              <?php if ($year != $startYear): ?>
              <li class="dropdown-divider"></li>
            <?php endif; ?>
        </li>
        <?php
